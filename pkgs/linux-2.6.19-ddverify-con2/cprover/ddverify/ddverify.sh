...
Usage: $SELF [OPTIONS] SOURCES ...
  ...
  SATABS, CBMC, or WOLVERINE
   
  Options for the DDVerify front end:   Purpose:
    -h|--help                           show help
    -D macro                            define preprocessor macro
    -o file                             file name of resulting binary (defaults to first SOURCE without .c)
    --ddv-path                          set ddverify path (defaults to path to $SELF)
    --kernel-src                        set kernel source directory (defaults to parent directory)
  ...
  Compiler selection:
    --goto-cc                           compile using goto-cc (default)
    --cil                               compile using CIL
    --cpp                               compile using gcc -E
  Driver description:
...
if [ ! -L $KERNEL_SRC/include/asm ] ; then
  link_exists=0
  ln -s asm-i386 $KERNEL_SRC/include/asm
...
EOF
  ...
  case "$COMPILER" in
    goto-cc)
      cat >> $OUTPUT_COMPILE_FILE <<EOF
$COMPILER -o $OUTPUT $preproc $all_src
EOF
      ;;
    cil) 
      cat >> $OUTPUT_COMPILE_FILE <<EOF
